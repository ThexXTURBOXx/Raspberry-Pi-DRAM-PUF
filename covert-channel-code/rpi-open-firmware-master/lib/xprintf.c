...
)
{
	...
	for (;;) {
		...
		case 'S' :					/* String */
			p = va_arg(arp, char*);
			for (j = 0; p[j]; j++) ;
			while (!(f & 2) && j++ < w) putcharBinary(' ');
			__puts(p);
			while (j++ < w) putcharBinary(' ');
			continue;
		case 'C' :					/* Character */
			putcharBinary((char)va_arg(arp, int)); continue;
		case 'B' :					/* Binary */
			r = 2; break;
		case 'O' :					/* Octal */
			r = 8; break;
		...
		if (f & 8) s[i++] = '-';
		j = i; d = (f & 1) ? '0' : ' ';
		while (!(f & 2) && j++ < w) putcharBinary(d);
		do putcharBinary(s[--i]); while(i);
		while (j++ < w) putcharBinary(' ');
	}
}
...
int printfBinary (		/* Put a formatted string to the default device */
	const char*	fmt,	/* Pointer to the format string */
	...					/* Optional arguments */
)
{
